...
from serenity.agent.tools.schema import BooleanSchema, IntegerSchema, StringSchema, tool_parameters_schema
from serenity.agent.tools import file_state
from serenity.utils.helpers import build_image_content_blocks, detect_image_mime
...
class WriteFileTool(_FsTool):
    ...
    async def execute(self, path: str | None = None, content: str | None = None, **kwargs: Any) -> str:
        try:
            ...
            return f"Successfully wrote and verified {len(written)} chars to {fp} ✓"
        except PermissionError as e:
            return f"Error: {e}"
        except Exception as e:
...
@tool_parameters(
    tool_parameters_schema(
        path=StringSchema("Path of the directory to create. Supports ~ for home directory."),
        required=["path"],
    )
)
class MakeDirTool(_FsTool):
    ...
    def description(self) -> str:
        return (
            "Create a folder at the given path, including any missing parent folders. "
            "Accepts ~ for the home directory. "
            "Examples: make_dir('~/Desktop/MyFolder'), make_dir('~/Documents/Projects'). "
            "Use this instead of exec/mkdir to avoid shell quoting issues."
        )
