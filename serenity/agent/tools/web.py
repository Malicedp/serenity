...
from serenity.utils.helpers import build_image_content_blocks

if TYPE_CHECKING:
    from serenity.config.schema import WebSearchConfig
...
_MAX_SNIPPET_CHARS = 400  # ~100 tokens per result — keeps web search lean
...
class WebSearchTool(Tool):
    ...
    async def execute(self, query: str, count: int | None = None, **kwargs: Any) -> str:
        # Always resolve through _effective_provider() so fallback logic
        # (missing API key → DuckDuckGo) is applied consistently.
        ...
        if provider == "duckduckgo":
            ...
            # Unknown provider — fall back to DuckDuckGo rather than hard error
            logger.warning("Unknown search provider '{}', falling back to DuckDuckGo", provider)
            return await self._search_duckduckgo(query, n)
        ...
        try:
            async with httpx.AsyncClient(proxy=self.proxy, follow_redirects=True, max_redirects=MAX_REDIRECTS, timeout=15.0) as client:
                async with client.stream("GET", url, headers={"User-Agent": USER_AGENT}) as r:
                    from serenity.security.network import validate_resolved_url
