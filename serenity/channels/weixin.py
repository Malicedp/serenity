...
from serenity.config.paths import get_media_dir, get_runtime_subdir
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.serenity/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'serenity channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"serenity-{uuid.uuid4().hex[:12]}"
