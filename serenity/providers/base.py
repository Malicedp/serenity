...
class LLMProvider(ABC):
    ...
    # Phrases that indicate the model ran out of RAM / VRAM.
    # Retrying is futile — memory won't free in the few seconds between attempts.
    ...
    @classmethod
    def _is_oom_error(cls, response: "LLMResponse") -> bool:
        """Return True if the error is an out-of-memory / insufficient RAM condition.

        These are never worth retrying — system RAM won't free up in the 1-4 s
        between retry attempts.  Fail fast so the user gets a quick, clear message.
        """
        err = (response.content or "").lower()
        return any(m in err for m in cls._OOM_ERROR_MARKERS)
    ...
    async def _run_with_retry(
        ...
        *,
        ...
        while True:
            ...
            # Timeout errors: never retry. The prompt that caused a timeout will
            # cause the next attempt to timeout too — retrying wastes 300s × N.
            # Log clearly so the user knows why and can reduce context / switch model.
            if response.error_kind == "timeout":
                logger.warning(
                    "LLM timed out — not retrying (same prompt will timeout again). "
                    "Consider lowering SERENITY_LLM_TIMEOUT or reducing context size."
                )
                return response

            # OOM errors: never retry. RAM won't free up in the 1-4s retry window.
            # Fail fast so the user gets a quick, actionable message.
            if self._is_oom_error(response):
                logger.warning(
                    "LLM out of memory — not retrying. "
                    "Close other applications or switch to a smaller model "
                    "(serenity → [M] Model & Provider)."
                )
                return response
            ...
            # 429 rate-limit: honour Retry-After if provided, otherwise wait 60s minimum.
            # NIM free tier needs ~60s to refill; 1-4s retries just cause more 429s.
