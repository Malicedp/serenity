...
from serenity.providers.base import LLMProvider, LLMResponse, ToolCallRequest
from serenity.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from serenity.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/danieltniamke/serenity",
    "X-OpenRouter-Title": "serenity",
...
# Ollama models that support the `think` flag (native chain-of-thought).
_OLLAMA_THINKING_PATTERNS: tuple[str, ...] = (
    "qwen3", "qwq",
    "deepseek-r1",
    "phi4-reasoning",
    "granite3-dense",   # IBM Granite reasoning variants
    "exaone3.5",        # LG AI Research reasoning model
)

# Cloud / OpenAI-compatible models that accept `reasoning_effort`.
_CLOUD_REASONING_PATTERNS: tuple[str, ...] = (
    "o1", "o3", "o4",       # OpenAI reasoning family
    "gpt-5",                # GPT-5 reasoning
    "qwen-thinking",        # Qwen cloud reasoning variants
    "qwen3-235b",           # Qwen3 MoE cloud
    "deepseek-r1",          # DeepSeek R1 cloud
    "deepseek-v4",          # DeepSeek V4 (Flash + Pro) — thinking via reasoning_effort
    "deepseek-v3",          # DeepSeek V3.x — thinking via reasoning_effort
    "claude-3-7",           # Anthropic extended thinking
    "claude-3-5",           # Anthropic extended thinking (some variants)
    "claude-4",             # Future Anthropic
)


def _model_supports_ollama_thinking(model_name: str) -> bool:
    """Return True if this Ollama model supports the `think: true` flag."""
    name = model_name.lower().split("/")[-1]  # strip provider prefix
    return any(pat in name for pat in _OLLAMA_THINKING_PATTERNS)


def _model_supports_cloud_reasoning(model_name: str) -> bool:
    """Return True if this cloud model accepts `reasoning_effort`."""
    name = model_name.lower()
    return any(pat in name for pat in _CLOUD_REASONING_PATTERNS)
...
def _resolve_effort(
    ...
    """Translate `reasoning_effort` into concrete API values.

    Returns ``(api_effort, ollama_think)`` where:
    - ``api_effort``   — value to pass as ``reasoning_effort`` to cloud APIs
                         (None = omit the field)
    - ``ollama_think`` — value to pass as ``think`` to Ollama
                         (None = omit the field)

    "adaptive" means "enable thinking if the model supports it, otherwise skip".
    It is never forwarded to any external API as-is.
    """
    ...
    if is_ollama:
        # Ollama uses think=true/false, never reasoning_effort.
        if effort_lower == "none":
            return None, False
        if effort_lower == "adaptive":
            supports = _model_supports_ollama_thinking(model_name)
            return None, True if supports else None
        # Explicit high/medium/low → enable thinking if model supports it.
        if effort_lower in ("high", "medium", "low"):
            supports = _model_supports_ollama_thinking(model_name)
            return None, True if supports else None
        return None, None
    ...
    # Cloud provider
    if effort_lower == "none":
        return None, None   # "none" = omit reasoning_effort entirely (Groq/etc reject the field)
    if effort_lower == "adaptive":
        if _model_supports_cloud_reasoning(model_name):
            return "high", None     # adaptive → full thinking on cloud reasoning models
        return None, None           # model doesn't support it — omit
    if effort_lower in ("high", "medium", "low", "minimal"):
        return reasoning_effort, None
    # Unknown value — drop it rather than send garbage to the API
    return None, None
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply Serenity attribution headers to OpenRouter requests by default."""
...
# Cache: "base_url|model_name" → num_ctx int. Populated lazily on first use.
_ollama_ctx_cache: dict[str, int] = {}


def _query_ollama_ctx(api_base: str, model_name: str, cap: int) -> int:
    """Ask Ollama what context window the model actually supports.

    Queries POST /api/show and reads the architecture's context_length field.
    Returns min(model_actual, cap) so the config value acts as a ceiling.
    Falls back to cap if Ollama is unreachable or the field is missing.
    """
    cache_key = f"{api_base}|{model_name}"
    ...
    try:
        ...
        # Architecture keys vary (llama.context_length, mistral.context_length, etc.)
        ctx_key = next((k for k in model_info if k.endswith(".context_length")), None)
        if ctx_key:
            model_ctx = int(model_info[ctx_key])
            result = min(model_ctx, cap)
            _ollama_ctx_cache[cache_key] = result
            import logging
            logging.getLogger(__name__).debug(
                f"Ollama {model_name}: context_length={model_ctx}, using num_ctx={result}"
            )
            return result
    except Exception:
        pass  # Ollama unreachable or model not loaded yet — fall back to cap

    _ollama_ctx_cache[cache_key] = cap
    return cap
...
class OpenAICompatProvider(LLMProvider):
    ...
    def __init__(
        ...
        context_window_tokens: int = 40960,
    ):
        ...
        # Timeout for local providers (Ollama) — long enough for a slow model
        # to finish generating, short enough that a hung model doesn't block forever.
        # SERENITY_LLM_TIMEOUT overrides (in seconds). Default: 300s.
        # 300s gives Qwen3/deepseek-r1 thinking models enough headroom to reload
        # from VRAM if evicted by the embedding model (nomic-v1.5) during NNN ops,
        # then complete a full thinking pass without timing out.
        _llm_timeout = float(os.environ.get("SERENITY_LLM_TIMEOUT", "300"))
    ...
    def _build_kwargs(
        ...
        # Ollama defaults to num_ctx=2048 — far too small for tool use.
        # Query the model's actual context_length and use that, capped at the
        # user's contextWindowTokens setting so they can still limit for speed.
        _is_ollama = spec and spec.is_local and "11434" in (self.api_base or "")
        _ollama_extra: dict[str, Any] = {}
        if _is_ollama:
            num_ctx = _query_ollama_ctx(
                self.api_base or "http://localhost:11434/v1",
                model_name,
                self._context_window_tokens,
            )
            _ollama_opts: dict[str, Any] = {"num_ctx": num_ctx}
            _ollama_extra = {"options": _ollama_opts}

        # Resolve "adaptive" (and any other effort value) to concrete API params.
        # _api_reasoning_effort → passed as reasoning_effort to cloud APIs
        # _ollama_think         → passed as think=true/false to Ollama
        _api_reasoning_effort, _ollama_think = _resolve_effort(
            reasoning_effort, model_name, bool(_is_ollama)
        )
        if _ollama_think is not None:
            _ollama_extra["think"] = _ollama_think
        ...
        # reasoning_effort is active.  Use the resolved API value — if we're not
        # actually sending reasoning_effort to this model, temperature is fine.
        if self._supports_temperature(model_name, _api_reasoning_effort):
        ...
        if _api_reasoning_effort:
            kwargs["reasoning_effort"] = _api_reasoning_effort
        ...
        if _ollama_extra:
            kwargs.setdefault("extra_body", {}).update(_ollama_extra)
    ...
    def _build_responses_body(
        ...
        _resolved_effort, _ = _resolve_effort(reasoning_effort, model_name, is_ollama=False)
        if _resolved_effort and _resolved_effort != "none":
            body["reasoning"] = {"effort": _resolved_effort}
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("SERENITY_STREAM_IDLE_TIMEOUT_S", "150"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
