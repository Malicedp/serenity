...
def get_config_path() -> Path:
    ...
    return Path.home() / ".serenity" / "config.json"
...
def load_config(config_path: Path | None = None) -> Config:
    ...
    config = Config()
    if path.exists():
        # Strip Windows BOM from the file once if present, so future reads with
        # any encoder work fine (Notepad & some editors write UTF-8 with BOM).
        _strip_bom_inplace(path)
        try:
...
def _strip_bom_inplace(path: Path) -> None:
    """Remove UTF-8 BOM from *path* in-place if present. No-op otherwise."""
    BOM = b"\xef\xbb\xbf"
    try:
        raw = path.read_bytes()
        if raw.startswith(BOM):
            path.write_bytes(raw[len(BOM):])
            logger.info("config.json: stripped UTF-8 BOM — file is now clean")
    except Exception:
...
def _apply_ssrf_whitelist(config: Config) -> None:
    ...
    from serenity.security.network import configure_ssrf_whitelist
