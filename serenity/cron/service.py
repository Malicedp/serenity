...
from serenity.cron.types import CronJob, CronJobState, CronPayload, CronRunRecord, CronSchedule, CronStore
...
class CronService:
    ...
    # Job kinds that survive a restart — everything else is session-scoped and purged.
    _PERSISTENT_JOB_IDS = frozenset({"dream"})
    _PERSISTENT_PAYLOAD_KINDS = frozenset({"system_event"})

    async def start(self) -> None:
        """Start the cron service.

        On every startup, session-scoped jobs (agent_turn payloads not in the
        persistent allow-list) are purged. This prevents stale session-scoped
        jobs from firing when the user opens a fresh session.
        """
        self._running = True
        self._load_store()

        # ── Purge session-scoped jobs ─────────────────────────────────────────
        if self._store:
            before = len(self._store.jobs)
            self._store.jobs = [
                j for j in self._store.jobs
                if (
                    j.id in self._PERSISTENT_JOB_IDS
                    or j.payload.kind in self._PERSISTENT_PAYLOAD_KINDS
                )
            ]
            purged = before - len(self._store.jobs)
            if purged:
                logger.info("Cron startup: purged {} session-scoped job(s)", purged)
        # ─────────────────────────────────────────────────────────────────────

        # ── Purge expired one-shot ("at") jobs ────────────────────────────────
        # A one-shot job whose scheduled time is in the past will never fire
        # (next_run_at_ms = None) and never self-delete — it becomes a zombie.
        # Delete them on startup so stale tasks don't linger in the cron list.
        if self._store:
            now = _now_ms()
            before = len(self._store.jobs)
            self._store.jobs = [
                ...
                if not (
                    j.schedule.kind == "at"
                    and j.schedule.at_ms is not None
                    and j.schedule.at_ms <= now
                )
            ]
            expired = before - len(self._store.jobs)
            if expired:
                logger.info("Cron startup: purged {} expired one-shot job(s)", expired)
        # ─────────────────────────────────────────────────────────────────────
    ...
    # Hard cap per job — prevents one slow LLM call from freezing the entire
    # cron loop. Dream (no LLM) is fast; user-scheduled jobs get 10 min max.
    _JOB_TIMEOUT_S: float = float(__import__("os").environ.get("SERENITY_CRON_JOB_TIMEOUT", "600"))
    ...
    async def _execute_job(self, job: CronJob) -> None:
        ...
        try:
            ...
            job.state.last_error = f"timed out after {int(self._JOB_TIMEOUT_S)}s"
            logger.error(
                "Cron: job '{}' timed out after {}s — cron loop unblocked",
                job.name, int(self._JOB_TIMEOUT_S),
            )
        except Exception as e:
            job.state.last_status = "error"
    ...
    def add_job(
        ...
        # Reject past-due one-shot jobs immediately — they can never fire and
        # would otherwise become zombies (next_run_at_ms=None, never cleaned up).
        if schedule.kind == "at" and job.state.next_run_at_ms is None:
            logger.warning(
                "Cron: rejected past-due one-shot job '{}' (scheduled at was in the past)",
                name,
            )
            return job  # return the job object but don't persist it
