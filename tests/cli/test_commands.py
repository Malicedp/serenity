...
from serenity.cli.commands import _make_provider, app
from serenity.config.schema import Config
from serenity.cron.types import CronJob, CronPayload
from serenity.providers.openai_codex_provider import _strip_model_prefix
...
def mock_paths():
    ...
    with patch("serenity.config.loader.get_config_path") as mock_cp, \
         patch("serenity.config.loader.save_config") as mock_sc, \
         patch("serenity.config.loader.load_config") as mock_lc, \
         patch("serenity.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "Serenity is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from serenity.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "serenity.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("serenity.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'serenity agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"serenity gateway --config {resolved_config}" in compact_output
...
def test_openai_compat_provider_passes_model_through():
    ...
    with patch("serenity.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_make_provider_uses_github_copilot_backend():
    from serenity.cli.commands import _make_provider
    from serenity.config.schema import Config
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("serenity.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
        provider = GitHubCopilotProvider(default_model="github-copilot/gpt-5.1")
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("serenity.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("serenity.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("serenity.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("serenity.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("serenity.cli.commands._make_provider", return_value=object()), \
         patch("serenity.cli.commands._print_agent_response") as mock_print_response, \
         patch("serenity.bus.queue.MessageBus"), \
         patch("serenity.cron.service.CronService"), \
         patch("serenity.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "serenity.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("serenity.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("serenity.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("serenity.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("serenity.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("serenity.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("serenity.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("serenity.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("serenity.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("serenity.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("serenity.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("serenity.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("serenity.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("serenity.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("serenity.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("serenity.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("serenity.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("serenity.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("serenity.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("serenity.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "serenity.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("serenity.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("serenity.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "serenity.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "serenity.cli.commands._make_provider",
        make_provider or (lambda _config: object()),
    )

    if message_bus is not None:
        monkeypatch.setattr("serenity.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("serenity.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("serenity.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("serenity.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("serenity.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("serenity.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("serenity.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("serenity.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("serenity.cli.commands._make_provider", lambda _config: provider)
    monkeypatch.setattr("serenity.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("serenity.session.manager.SessionManager", lambda _workspace: object())
    ...
    monkeypatch.setattr("serenity.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("serenity.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("serenity.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "serenity.utils.evaluator.evaluate_response",
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from serenity.cli.commands import _migrate_cron_store
    ...
    with patch("serenity.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("serenity.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("serenity.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("serenity.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("serenity.heartbeat.service.HeartbeatService", _FakeHeartbeatService)
