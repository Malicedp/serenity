...
from serenity.command.builtin import cmd_new, register_builtin_commands
from serenity.command.router import CommandContext, CommandRouter
from serenity.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("serenity.agent.loop.SessionManager"), \
         patch("serenity.agent.loop.SubagentManager") as MockSubMgr, \
         patch("serenity.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates serenity onboard)."""
        from serenity.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from serenity.agent.memory import Consolidator, MemoryStore
