...
try:
    from serenity.channels import dingtalk
...
import serenity.channels.dingtalk as dingtalk_module
from serenity.channels.dingtalk import DingTalkChannel, SerenityDingTalkHandler
from serenity.channels.dingtalk import DingTalkConfig
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "Serenity Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = SerenityDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/serenity_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/serenity_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "serenity.config.paths.get_media_dir",
