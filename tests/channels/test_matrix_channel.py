...
from serenity.channels.matrix import _build_matrix_text_content

import serenity.channels.matrix as matrix_module
from serenity.bus.events import OutboundMessage
from serenity.bus.queue import MessageBus
from serenity.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from serenity.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("serenity.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "serenity.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("serenity.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "serenity.channels.matrix.asyncio.create_task", _fake_create_task
    )
