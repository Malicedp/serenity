...
If you discover a security vulnerability in serenity, please report it by:
...
chmod 600 ~/.serenity/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.serenity/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.serenity/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run serenity with a dedicated user account
...
- Keep authentication data in `~/.serenity/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for serenity and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash serenity
   sudo -u serenity serenity gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.serenity
   chmod 600 ~/.serenity/config.json
   chmod 700 ~/.serenity/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.serenity/logs/serenity.log
   ```
...
- **Chat history is stored locally** - protect the `~/.serenity` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.serenity/logs/serenity.log
   ```
...
- GitHub Security Advisories: https://github.com/danieltniamke/serenity/security/advisories
- Release Notes: https://github.com/danieltniamke/serenity/releases
