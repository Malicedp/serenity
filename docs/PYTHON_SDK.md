...
Use serenity programmatically — load config, run the agent, get results.
...
### `Serenity.from_config(config_path?, *, workspace?)`

Create a `Serenity` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.serenity/config.json`. |
...
```python
from serenity.agent import AgentHook, AgentHookContext
...
async def main():
    bot = Serenity.from_config(workspace="/my/project")
