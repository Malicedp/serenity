...
Build a custom serenity channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of serenity (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

serenity discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `serenity gateway` starts, it scans:

1. Built-in channels in `serenity/channels/`
2. External packages registered under the `serenity.channels` entry point group
...
```
serenity-channel-webhook/
├── serenity_channel_webhook/
...
```python
# serenity_channel_webhook/__init__.py
from serenity_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# serenity_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "serenity-channel-webhook"
version = "0.1.0"
dependencies = ["serenity", "aiohttp"]

[project.entry-points."serenity.channels"]
webhook = "serenity_channel_webhook:WebhookChannel"
...
packages = ["serenity_channel_webhook"]
```
...
```bash
pip install -e .
serenity plugins list      # verify "Webhook" shows as "plugin"
serenity onboard           # auto-adds default config for detected plugins
```

Edit `~/.serenity/config.json`:
...
```bash
serenity channels login <channel_name>
serenity channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `serenity onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `serenity.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `serenity.config.schema.Base`:
...
Override `default_config()` so `serenity onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `serenity-channel-{name}` | `serenity-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `serenity_channel_{name}` | `serenity_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/serenity-channel-webhook
cd serenity-channel-webhook
pip install -e .
serenity plugins list    # should show "Webhook" as "plugin"
serenity gateway         # test end-to-end
```

## Verify

```bash
$ serenity plugins list
